...
% Add bias ones to the h1 data matrix
a2=[ones(m, 1) a2];
...
% Feedforward passed in lambda as 0, therefore not used

%% Regularized cost
Theta1s=Theta1(:,2:end);
Theta2s=Theta2(:,2:end);
J=(1/m).* sum(sum((-y).*log(a3)-(1-y).*log(1-a3))) + (lambda / 2 / m) .* (sum(sum(Theta1s.^2)) + sum(sum(Theta2s.^2)));
...
Theta1_grad=(1/m).*tridelta_1 + (lambda/m) * [zeros(size(Theta1, 1), 1) Theta1s];
Theta2_grad=(1/m).*tridelta_2 + (lambda/m) * [zeros(size(Theta2, 1), 1) Theta2s];
